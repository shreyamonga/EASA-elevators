...
storeApplication(customer: any,appEdit:boolean) {
  ...
  deletereport(id: any) {
    const params = new HttpParams()
      .set('id', id.toString());
    return this.http.post(`${this.baseUrl2}/lead/filter_page/report_delete`, { id: id }, { 'headers': this.getHeader() }).pipe(
      map((res: any) => {
        return res;
      })
    );
...
replaceKeyInArray(arr: any[], oldKey: string, newKey: string): any[] {
  ...
  reportDetails(id:any,pagination:any,searchValue:any,datefilter:any){
  ...
  getReportList() {
    return this.http.get(`${this.baseUrl2}/lead/filter_page/report_all`, { 'headers': this.getHeader() }).pipe(
      map((res: any) => {
        // //console.log(res)
        return res;
      })
