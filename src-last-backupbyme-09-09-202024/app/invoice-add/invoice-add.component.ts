  ...
  Type:any = '';
        ...
        (data: Orders[]) => {
          ...
          for (let i = 0; i < this.quotations[0].DocumentLines.length; i++) {
            this.QuatItems.push({
              ...
              Tax: this.quotations[0].DocumentLines[i].Tax||0,
            });
          }
    ...
    openPrviewLg(content: any, f: NgForm) {
      ...
      for (let i = 0; i < this.previewData.DocumentLines.length; i++) {
        ...
        var total = aftersdis + (aftersdis * (Number(this.previewData['DocumentLines'][i].Tax) / 100))
        totalamount += total;
        this.total_afterpreview += afterfdis;
        this.total_after_taxpreview += aftersdis;
        this.tax_Valuepreview +=  (aftersdis * (Number(this.previewData['DocumentLines'][i].Tax) / 100));
      }
