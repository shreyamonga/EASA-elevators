...
export class SourcesComponent implements OnInit {
  ...
  getIndustryList(): void {
    ...
    this.bridgeService2.getAllSourcedata().subscribe(
      // this.bridgeService2.getAllSourcedata(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
        this.industry = data;
  ...
  addIndustry(f: NgForm) {
    ...
    if (f.valid) {
      this.isLoading = true;
      this.bridgeService2.AddEditLeadSource(this.indus,this.isEdit).subscribe(
        (res: Industry) => {
          if (Object(res)['status'] == "200") {
            ...
            this._NotifierService.showSuccess(this.isEdit ? 'Industry Updated Successfully !' : 'Industry Added Successfully !')
          }
          else{
      ...
      for (let i = 0; i < Object.keys(f.value).length; i++) {
        var keyys = Object.keys(f.value)[i];
        if (!f.value[keyys] || f.value[keyys].length === 0) {
