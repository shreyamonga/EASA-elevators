...
export class PaymentCollactionComponent implements OnInit {
  ...
  savedModules: any[] = [];
        ...
        (res: any) => {
          if (Object(res)['status'] == "200") {
            ...
            for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
              this.pyaddterms[this.DynamicFiledPositionDetials[i].field_name] = '';
            }
          }
          else {
    ...
    onFileChanged2(event: any) {
      ...
      if (confirm('Are you sure you want to add this attachment?')) {
        ...
        this.Attachment.CreateDate = this.HeadingServices.getDate();

        this.Attachment.CreateTime = this.HeadingServices.getTime();

        this.Attachment.UpdateDate = this.HeadingServices.getDate();

        this.Attachment.UpdateTime = this.HeadingServices.getTime();
    ...
    selectChangeHandlerItem(event: any) {
      ...
      this.pyaddterms.InvoiceNo =  "";
      this.pyaddterms.TotalAmt =  "";
      this.pyaddterms.DueAmount = ""
      this.getInvoice();
    }
    selectChangeHandlerItem2(event: any) {
      ...
      this.bridgeService2.getOneInvoicedata(event).subscribe(
        (data: any[]) => {
          this.pyaddterms.CardCode = data[0].CardCode;
          this.pyaddterms.TotalAmt =  Number(data[0].due_amount).toFixed(2);
          this.pyaddterms.DueAmount = Number(this.pyaddterms.TotalAmt) - Number(this.pyaddterms.ReceivedAmount)
          this.pyaddterms.DueAmount = this.pyaddterms.DueAmount.toFixed(2);
        },
        (err) => {
          console.log(err);
        }
