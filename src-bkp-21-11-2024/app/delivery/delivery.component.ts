...
export class DeliveryComponent implements OnInit {
  ...
  savedModules: any[] = [];
  constructor(private modalService: NgbModal, private route: Router,public HeadingServices: HeadingServicesService,
      ...
      (res: any) => {
        if (Object(res)['status'] == "200") {
          ...
          for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
            this.AddDelivery[this.DynamicFiledPositionDetials[i].field_name] = '';
          }
        }
        else {
...
bigScreenOrMid() {
  ...
  SaveOrder(f: NgForm) {
    ...
    if (f.valid) {
      ...
      this.AddDelivery.UpdateTime = this.HeadingServices.getTime2();
      this.AddDelivery.UpdateTime = this.HeadingServices.getTime();
      this.AddDelivery.CreateDate = this.HeadingServices.getDate();
