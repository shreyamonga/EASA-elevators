...
export class PaymentmasterComponent implements OnInit {
  ...
  getPyterms(): void {
    ...
    this.bridgeService2.getPaymentTermsByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
      ...
      this.bridgeService2.insertPaymentTerms(this.pyaddterms,this.isEdit).subscribe(
        (res: PaymentTerms) => {
          if (Object(res)['status'] == "200") {
            ...
            this._NotifierService.showSuccess(this.isEdit ? 'Payment Master Updated Successfully !' : 'Payment Master Added Successfully !')
          }
          else{
            this._NotifierService.showError(Object(res)['message']);
            this.isLoading = false;
