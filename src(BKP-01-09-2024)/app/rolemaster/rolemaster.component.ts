...
export class RolemasterComponent implements OnInit {
      ...
      ngOnInit(): void {
        ...
        this.bridgeService2.getdepartmentsMasterPagination({PageNo: 1,maxItem: 'All'},'','id','asc').subscribe(
          (data: any) => {
      ...
      getCustomerTypeList(): void {
        ...
        this.bridgeService2.getRoleMasterPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
          (data: any) => {
  ...
  this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', modalDialogClass: 'modal-dialog-centered order-cards-modal' }).result.then((result) => {
    ...
    this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
      ...
      addCustomerType(f: NgForm) {
        ...
        this.resetAlerts();
        if (f.valid) {
          this.bridgeService2.addEditRoleMaster(this.type,this.isEdit).subscribe(
            (res: any) => {
