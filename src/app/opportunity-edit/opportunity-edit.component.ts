...
export class OpportunityEditComponent implements OnInit {
  ...
  constructor(
    ...
    private http: HttpClient, private _NotifierService: NotiferService,public HeadingServices: HeadingServicesService,
      ...
      (res: any) => {
        if (Object(res)['status'] == "200") {
          ...
          for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
            this.opportunity[this.DynamicFiledPositionDetials[i].field_name] = '';
          }
          this.getOpportunity();
        }
        else {
    ...
    var totalamount:any = 0;
       ...
       // console.log(this.opportunity.U_LSOURCE);
        ...
        for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
          ...
          if(this.opportunitys[0][this.DynamicFiledPositionDetials[i].field_name] == null){
            this.opportunitys[0][this.DynamicFiledPositionDetials[i].field_name] = '';
          }
          this.opportunity[this.DynamicFiledPositionDetials[i].field_name] = this.opportunitys[0][this.DynamicFiledPositionDetials[i].field_name];
        }
...
showitem() {
  ...
  EditOpportunity(f: NgForm) {
    ...
    if(f.valid){
      ...
      this.opportunity.StartDate= this.HeadingServices.getDate(),
      this.opportunity.UpdateDate=this.HeadingServices.getDate(),
      this.opportunity.UpdateTime=this.HeadingServices.getTime(),
