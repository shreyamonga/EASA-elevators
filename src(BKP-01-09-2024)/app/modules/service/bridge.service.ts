...
storeApplication(customer: any,appEdit:boolean) {
  ...
  loginFunctionbyToken(rememberMe:any,logindata:any) {
    ...
    return this.http.get(`${this.baseUrl2}/employee/get_user_info`, { 'headers': this.getHeader() }).pipe(
      map((res: any) => {
        if (rememberMe) {
          ...
          localStorage.setItem('currentUserName', logindata.email);
          localStorage.setItem('currentUserPassword', logindata.password);
        }
        else{
