...
@Component({
  ...
  styleUrls: ['./opportunitytype.component.css']
})
export class OpportunitytypeComponent implements OnInit {
  ...
  selectAll1() {
    ...
    if (num.checked) {
      this.count = [];
      for (let i = 0; i < this.optype.length; i++) {
        this.count.push(this.optype[i].id);
      }
  ...
  getOpportunityList(): void {
    ...
    this.bridgeService2.getOppTypeByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
        this.optype = data.data;
  ...
  addIndustry(f: NgForm) {
    ...
    if (f.valid) {
      this.isLoading = true;
      this.bridgeService2.insertOpportunityType(this.type,this.isEdit).subscribe(
        (res: coType) => {
