...
export class CustomerAddComponent implements OnInit {
      ...
      (res: any) => {
        if (Object(res)['status'] == "200") {
          ...
          for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
            this.customer[this.DynamicFiledPositionDetials[i].field_name] = '';
          }
        }
        else {
  ...
  addCustomer(f: NgForm) {
    ...
    this.customer.UpdateTime = this.HeadingServices.getTime2();
    this.customer.UpdateTime = this.HeadingServices.getTime();
    this.customer.CreateDate = this.HeadingServices.getDate();
        if (f.valid) {
