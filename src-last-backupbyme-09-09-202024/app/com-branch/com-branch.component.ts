...
@Component({
  ...
  templateUrl: './com-branch.component.html',
  styleUrls: ['./com-branch.component.scss']
})
export class ComBranchComponent implements OnInit {
      ...
      getState(event:any){
        this.bridgeService2.getStatedata(event).subscribe(
          (data: States[]) => {
            this.statess = data;
          },
          (err) => {
      ...
      getCustomerTypeList(): void {
        ...
        this.bridgeService2.getBranchMasterPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
          (data: any) => {
      ...
      addCustomerType(f: NgForm) {
        ...
        this.resetAlerts();
        if (f.valid) {
          this.bridgeService2.addEditBranchMaster(this.type,this.isEdit).subscribe(
            (res: any) => {
