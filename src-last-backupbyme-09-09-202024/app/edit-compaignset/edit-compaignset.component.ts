...
export class EditCompaignsetComponent implements OnInit {
  ...
  constructor(private _location: Location, private route: Router, private router: ActivatedRoute, private bridgeService: BridgeService, private modalService: NgbModal,
    ...
    this.bridgeService.getEditCampaignSetdata(this.idd).subscribe(
      (data: CampaignSet[]) => {
        ...
        if (this.compaigns.LeadSource != '') {
          ...
          this.selectleadsource = objectDays
        }
        else {
        ...
        if (this.compaigns.LeadStatus != '') {
          ...
          this.selectleadstatus = objectStatus
        }
        else {
