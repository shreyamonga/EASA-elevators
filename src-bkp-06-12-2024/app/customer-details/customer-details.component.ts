...
export class CustomerDetailsComponent implements OnInit {
      ...
      (res: any) => {
        if (Object(res)['status'] == "200") {
          ...
          for(let i=0;i<this.DynamicFiledPositionDetials.length;i++){
            this.customers[this.DynamicFiledPositionDetials[i].field_name] = '';
          }
        }
        else {
 ...
 yesdeleteBranch1(deletebranch1:any) {
  ...
  prepareFilesList(files: Array<any>) {
    ...
    this.cutomerAttach1.CreateTime = this.HeadingServices.getTime();
    this.cutomerAttach1.CreateDate = this.HeadingServices.getDate();
